...
export default function EditDogPage() {
  ...
  const [basics, setBasics] = useState({
    ...
    sex: "" as "male" | "female" | "",
    spayed_neutered: false, coat_color: "", photo_url: "",
  });
  ...
  const [safety, setSafety] = useState({
    ...
    medications: [] as { name: string; dose: string; frequency: string }[],
  });

  const [routine, setRoutine] = useState({
    walk_schedule: "", potty_schedule: "", crate_trained: false,
    sleep_location: "", exercise_level: "" as "low" | "medium" | "high" | "",
    feeding_schedule: [{ time: "", amount: "", food_brand: "" }] as { time: string; amount: string; food_brand: string }[],
  });

  const [behavior, setBehavior] = useState({
    good_with_kids: "" as "yes" | "no" | "caution" | "",
    good_with_dogs: "" as "yes" | "no" | "caution" | "",
    good_with_cats: "" as "yes" | "no" | "caution" | "",
  ...
  useEffect(() => {
    async function load() {
      ...
      const [dogRes, safetyRes, routineRes, behaviorRes] = await Promise.all([
      ...
      setBasics({
        ...
        weight_lbs: dog.weight_lbs ? String(dog.weight_lbs) : "",
        sex: dog.sex ?? "",
      ...
      if (safetyRes.data) {
        ...
        setSafety({
          ...
          vet_name: s.vet_name ?? "",
          vet_phone: s.vet_phone ?? "",
          vet_address: s.vet_address ?? "",
          microchip_number: s.microchip_number ?? "",
      ...
      if (routineRes.data) {
        ...
        setRoutine({
          walk_schedule: r.walk_schedule ?? "",
          potty_schedule: r.potty_schedule ?? "",
          crate_trained: r.crate_trained ?? false,
          sleep_location: r.sleep_location ?? "",
          exercise_level: r.exercise_level ?? "",
          feeding_schedule: r.feeding_schedule?.length ? r.feeding_schedule : [{ time: "", amount: "", food_brand: "" }],
        });
      }

      if (behaviorRes.data) {
        const b = behaviorRes.data;
        setBehavior({
          good_with_kids: b.good_with_kids ?? "",
          good_with_dogs: b.good_with_dogs ?? "",
          good_with_cats: b.good_with_cats ?? "",
          known_fears: b.known_fears ?? "",
          known_commands: b.known_commands ?? "",
          leash_behavior: b.leash_behavior ?? "",
  ...
  async function handleSave() {
    if (!basics.name.trim()) { setError("Dog's name is required."); return; }
    setSaving(true);
    setError(null);

    let photo_url = basics.photo_url;
    ...
    await Promise.all([
      ...
      supabase.from("dog_safety").upsert({
        dog_id: id,
        emergency_contact_name: safety.emergency_contact_name || null,
        emergency_contact_phone: safety.emergency_contact_phone || null,
        vet_name: safety.vet_name || null,
        vet_phone: safety.vet_phone || null,
        vet_address: safety.vet_address || null,
        microchip_number: safety.microchip_number || null,
        medical_conditions: safety.medical_conditions || null,
        allergies: safety.allergies || null,
        medications: safety.medications,
        updated_at: new Date().toISOString(),
      }, { onConflict: "dog_id" }),

      supabase.from("dog_routine").upsert({
        dog_id: id,
      ...
      supabase.from("dog_behavior").upsert({
        dog_id: id,
    ...
    window.location.href = `/dashboard/dog/${id}`;
  }
    ...
    <div className="min-h-screen bg-cream">
      <header className="sticky top-0 z-50 bg-navy py-[18px]">
        <div className="mx-auto flex max-w-[1100px] items-center justify-between px-6">
          ...
          <a href={`/dashboard/dog/${id}`} className="font-heading text-[0.9rem] text-cream/60 hover:text-cream">
            ← Cancel
          </a>
        </div>
      </header>
          ...
          {STEPS.map((label, i) => (
            <div key={label} className="flex items-center gap-2">
              <button onClick={() => setStep(i)}
                className={`flex h-8 w-8 items-center justify-center rounded-full font-heading text-[0.85rem] font-bold transition ${
                  i === step ? "bg-orange text-cream" : "bg-navy text-cream/60 hover:text-cream"
                }`}>
                {i + 1}
              </button>
              ...
              {i < STEPS.length - 1 && <div className="h-px w-6 bg-navy/20" />}
            </div>
          ))}
        </div>
            ...
            <div className="flex flex-col gap-5">
              ...
              <Field label="Dog Photo">
                <div className="flex items-center gap-4">
                  {(photoPreview || basics.photo_url) && (
                    <div className="relative h-16 w-16 overflow-hidden rounded-full border-2 border-orange">
                      <Image src={photoPreview ?? basics.photo_url} alt="Dog photo" fill className="object-cover" />
                    </div>
                  )}
                  <label className="cursor-pointer rounded-xl bg-slate/40 px-4 py-2.5 font-heading text-[0.9rem] text-cream/70 ring-1 ring-cream/10 transition hover:bg-slate/60 hover:text-cream">
                    {basics.photo_url || photoPreview ? "Change Photo" : "Upload Photo"}
